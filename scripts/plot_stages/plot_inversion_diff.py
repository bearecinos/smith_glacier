...
c = ax0.tricontourf(x_n, y_n, t, U_measures-U_itlive, levels = levels, cmap=cmap_vel, extend="both")
...
levels = np.linspace(minv,maxv,200)
ticks = np.linspace(minv,maxv,3)
c = ax1.tricontourf(x_n, y_n, t, uv_measures-uv_live, levels = levels, cmap=cmap_vel, extend="both")
smap.set_lonlat_contours(xinterval=1.0, yinterval=0.5, add_tick_labels=False, linewidths=1.5, alpha=0.3)
out = graphics.get_projection_grid_labels(smap)
smap.xtick_pos = out[0]
smap.xtick_val = out[1]
...
levels = np.linspace(minv,maxv,200)
ticks = np.linspace(minv,maxv,3)
c = ax2.tricontourf(x_n, y_n, t, alpha_v_me-alpha_v_il, levels=levels, cmap=cmap_params_alpha, extend="both")
