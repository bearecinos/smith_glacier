...
then
  ...
  cp $OUTPUT_DIR/02_gridded_data/smith_obs_vel_mix.h5 $input_run_inv/.
else
...
then
  ...
  mpirun -n 24 python $FENICS_ICE_BASE_DIR/runs/run_eigendec.py $RUN_CONFIG_DIR/run_inversion/smith.toml
  echo $(date -u) "Done!"
fi
