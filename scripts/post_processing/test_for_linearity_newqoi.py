...
parser.add_argument("-toml_path_workflow_m", type=str,
                    default="../run_experiments/run_workflow/smith_cloud.toml",
                    help="pass .toml file")
parser.add_argument("-toml_path_new_qoi", type=str,
                    default="../run_experiments/run_workflow/smith_cloud.toml",
                    help="pass .toml file")
...
# Toml for the new QoI estimate
tomlf_qoi =  args.toml_path_new_qoi
params_qoi = conf.ConfigParser(tomlf_qoi)
...
for n in num_sens:
    ...
    # My main question here is it is alpha_v_il+ (alpha_v_me - alpha_v_il)/100?
    # or just the line below
    dot_alpha_il = np.dot(dq_dalpha_il, (alpha_v_il - alpha_v_me)/100)
    dot_beta_il = np.dot(dq_dbeta_il, (beta_v_il - beta_v_me)/100)
...
t_sens = np.flip(np.linspace(params_workflow.time.run_length, 0, params_workflow.time.num_sens))

## Lets do the linear interpolation for the dot products
## Its the same time step for all the runs
dot_alpha = np.interp(qoi_dict_il['x'], t_sens, results_dot_alpha)
dot_beta = np.interp(qoi_dict_il['x'], t_sens, results_dot_beta)

## Now we read the NEW qoi
new_qoi_outputdir = params_qoi.io.output_dir
phase_name_qoi = params_qoi.time.phase_name
run_name_qoi = params_qoi.io.run_name
phase_suffix_qoi = params_qoi.time.phase_suffix

file_qoi = "_".join((params_qoi.io.run_name+phase_suffix_qoi, 'Qval_ts.p'))

phase_name = params_workflow.time.phase_name
run_name = params_workflow.io.run_name

fwd_qoi = Path(new_qoi_outputdir) / phase_name_qoi / phase_suffix_qoi

new_qoi_f_paht = fwd_qoi / file_qoi
assert new_qoi_f_paht.is_file(), "File not found"

with open(new_qoi_f_paht, 'rb') as f:
    out = pickle.load(f)
Qoi = out[0]
time = out[1]
...
p2, = ax.plot(qoi_dict_il['x'], dot_alpha, color=color_palette[0], label='', linewidth=3)
p3, = ax.plot(qoi_dict_il['x'], dot_beta, color=color_palette[1], label='', linewidth=3)
...
plt.tight_layout()
plt.savefig(os.path.join(plot_path, 'linearity_test_qoi_new.png'), bbox_inches='tight')
