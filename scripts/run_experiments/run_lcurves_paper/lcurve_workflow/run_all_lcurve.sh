...
export run_inv_output_dir=$OUTPUT_DIR/09_lcurve_workflow
...
path_logs=$RUN_CONFIG_DIR/run_lcurves_paper/lcurve_workflow
...
#echo $(date -u) "Run eigen stages started" | mail -s "run eigen started" beatriz.recinos@ed.ac.uk
...
echo $(date -u) "Run error prop stages started" | mail -s "run errprop started" beatriz.recinos@ed.ac.uk
...
echo $(date -u) "Run inv_sigma stages started" | mail -s "run invsigma started" beatriz.recinos@ed.ac.uk
